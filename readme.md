...
| State management | `window.myState` global variable |
| Slow DB query | `setTimeout` before the query to "let the DB get ready" |
| Error handling | `console.log("error lol")` and move on |
| Real-time updates | `setInterval` refresh every 1 second |
| Deploy | Copy files to server, text users to refresh |
| Caching | Save to a `.txt` file and read it back |
